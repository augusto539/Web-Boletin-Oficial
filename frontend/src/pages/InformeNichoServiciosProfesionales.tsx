...
export default function InformeNichoServiciosProfesionales() {
  ...
  useEffect(() => {
    obtenerEntidadesNicho("servicios-profesionales").then(setDatos);
  }, []);
  ...
  async function descargar() {
    ...
    try {
      ...
      await exportarNichoServiciosProfesionalesPDF(datos.entidades);
      registrarDescarga(
          ...
          <Reveal delay={0.55}>
            <div className="mt-10 rounded-3xl bg-white p-6 shadow-sm sm:p-8">
              ...
              {CATEGORIAS.map((categoria) => {
                const entidadesCategoria = datos.entidades.filter((e) => e.categoria === categoria);
